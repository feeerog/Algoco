...
int main() {
    ...
    if (numAleatorios.is_open()) {
        for (int i = 0; i < 40000; ++i) {
            int numero = std::rand() % 40001 + 1;  // Números entre 1 y 40001
    ...
    if (numAscendentes.is_open()) {
        for (int numero = 1; numero <= 40000; ++numero) {
    ...
    if (numDescendentes.is_open()) {
        for (int numero = 40000; numero >= 1; --numero) {
    ...
    if (numAlternantes.is_open()) {
        for (int i = 1, j = 40000; i <= 20000 && j >= 20000; ++i, --j) {
            numAlternantes << i << " " << j << " ";
        }
