...
int main() {
    ...
    if (numAleatorios.is_open()) {
        for (int i = 0; i < 30000; ++i) {
            int numero = std::rand() % 30001 + 1;  // Números entre 1 y 30001
    ...
    if (numAscendentes.is_open()) {
        for (int numero = 1; numero <= 30000; ++numero) {
    ...
    if (numDescendentes.is_open()) {
        for (int numero = 30000; numero >= 1; --numero) {
    ...
    if (numAlternantes.is_open()) {
        for (int i = 1, j = 30000; i <= 15000 && j >= 15000; ++i, --j) {
            numAlternantes << i << " " << j << " ";
        }
