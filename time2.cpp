...
int main() {
    ...
    if (numAleatorios.is_open()) {
        for (int i = 0; i < 5000; ++i) {
            int numero = std::rand() % 5001 + 1;  // Números entre 1 y 5001
    ...
    if (numDescendentes.is_open()) {
        for (int numero = 5000; numero >= 1; --numero) {
    ...
    if (numAlternantes.is_open()) {
        for (int i = 1, j = 5000; i <= 2500 && j >= 2500; ++i, --j) {
            numAlternantes << i << " " << j << " ";
        }
